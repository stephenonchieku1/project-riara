...
CVE_DATABASE = {
    'ftp': [
        {'CVE_ID': 'CVE-2019-1234', 'description': 'FTP service is vulnerable to buffer overflow.', 'cvss_score': 7.5},
        {'CVE_ID': 'CVE-2021-5678', 'description': 'FTP service is vulnerable to command injection.', 'cvss_score': 8.0}
    ],
    'telnet': [
        {'CVE_ID': 'CVE-2018-9101', 'description': 'Telnet service is vulnerable to information disclosure.', 'cvss_score': 6.5}
    ],
    'http': [
        {'CVE_ID': 'CVE-2020-2345', 'description': 'HTTP service is vulnerable to XSS attacks.', 'cvss_score': 5.0},
        {'CVE_ID': 'CVE-2022-3456', 'description': 'HTTP service is vulnerable to SQL injection.', 'cvss_score': 9.0}
    ]
}
        ...
        for port, service in open_ports.items():
            ...
            if service.lower() in CVE_DATABASE:
                for cve in CVE_DATABASE[service.lower()]:
                    vulnerabilities.append({'port': port, 'service': service, 'CVE_ID': cve['CVE_ID'], 'description': cve['description'], 'cvss_score': cve['cvss_score']})
                    logging.warning(f"Potential vulnerability detected on {ip} - Port: {port}, Service: {service}, CVE: {cve['CVE_ID']}, Description: {cve['description']}, CVSS Score: {cve['cvss_score']}")
    except Exception as e:
...
def prioritize_vulnerabilities(vulnerabilities):
    ...
    Returns:
        list: A list of vulnerabilities sorted by risk (high to low).
    """
...
def main():
    ...
    if vulnerabilities:
        ...
        for vuln in prioritized_vulnerabilities:
            print(f"- Port {vuln['port']} ({vuln['service']}) is vulnerable: CVE {vuln['CVE_ID']} - {vuln['description']} (CVSS Score: {vuln['cvss_score']})")
    else:
